...
static const Para_t c_fault_para[1] = {
    {1000, 200, 10000, 100, 10000}   /* 判断关灯故障参数阈值 */
};

static INT8S _alarm_turn_off_scan(Alarm_t* thiz)
{
	DECL_PRIV(thiz, priv);
    ...
    if (priv->lamp < LAMP_NUM_MAX) {
        ...
        if (lamp_is_open(priv->lamp)) {                                        /* 开灯状态，就不用判断开灯故障，返回原来的报警状态 */
            return priv->status;
        }
        ...
        {	
            if (priv->status != 1) {
                if (priv->count++ > COUNT_MAX) {
                    priv->count  = 0;
                    priv->status = 1;
                    SYS_DEBUG("<alarm_turn_off trigger>\n");
                }
            }
        } else {
            if (priv->status == 1) {
                if (priv->count++ > COUNT_MAX) {
                    priv->count  = 0;
                    priv->status = 0;
                    SYS_DEBUG("<alarm_turn_off relieve>\n");
                }
            }
        }
    } else {
...
static INT8S _alarm_turn_off_check(Alarm_t* thiz)
{
	DECL_PRIV(thiz, priv);
...
static void _alarm_turn_off_destroy(Alarm_t* thiz)
{
	if (thiz != NULL) {
		//DECL_PRIV(thiz, priv);
		mem_free(thiz);
...
Alarm_t* alarm_turn_off_create(Alarm_id_e id)
{
	...
	if (thiz != NULL) {
		DECL_PRIV(thiz, priv);
        thiz->id       = id;
		thiz->scan     = _alarm_turn_off_scan;
		thiz->check    = _alarm_turn_off_check;
		thiz->destroy  = _alarm_turn_off_destroy;

        if (id == LAMP1_TURN_OFF_FAULT) {
