...
#if 0
extern double __daylen__(int year, int month, int day, double lon, double lat, double altit, int upper_limb);

extern int __sunriset__(int year, int month, int day, double lon, double lat, double altit, int upper_limb,
...
#endif /* if 0. 2015-5-23 16:16:43 syj */

/******************************************************************/
/* This function reduces any angle to within the first revolution */
/* by subtracting or adding even multiples of 360.0 until the     */
/* m_result is >= 0.0 and < 360.0                                   */
/******************************************************************/
 ...
 *
 **/
static int __sunriset__(int year, int month, int day, double lon, double lat, double altit, int upper_limb,
...
#endif /* if 0. 2015-5-23 15:27:37 syj */
...
//add by lijian
void calcSunTime(struct tm *cur_tm, double lon, double lat)
{

	struct tm *_tm = cur_tm;
	double rise, set;
#if 0
	double dwan, dusk;
#endif /* if 0. 2015-5-23 16:22:32 syj */
    
	sun_rise_set(_tm->tm_year + 1900, _tm->tm_mon + 1, _tm->tm_mday, lon, lat, &rise, &set);
	rise += 8.0;
	set += 8.0;

	_tm->tm_hour = (int) rise;
	_tm->tm_min = (rise - _tm->tm_hour) * 60.0;
	_tm->tm_sec = 0;
	sunrise = mktime(_tm);

	_tm->tm_hour = (int) set;
	_tm->tm_min = (set - _tm->tm_hour) * 60.0;
	_tm->tm_sec = 0;
	sunset = mktime(_tm);

#if 0
	civil_twilight(_tm->tm_year + 1900, _tm->tm_mon + 1, _tm->tm_mday, lon, lat, &dwan, &dusk);
	dwan += 8;
	dusk += 8;

	_tm->tm_hour = (int) dwan;
	_tm->tm_min = (dwan - _tm->tm_hour) * 60.0;
	_tm->tm_sec = 0;
	dawn = mktime(_tm);

	_tm->tm_hour = (int) dusk;
	_tm->tm_min = (dusk - _tm->tm_hour) * 60.0;
	dusk = mktime(_tm);
#endif /* if 0. 2015-5-23 16:22:25 syj */
}

void SunSet(struct tm* cur_tm, double lon, double lat) {
	calcSunTime(cur_tm, lon, lat);
}

time_t getSunrise(void) {
	return sunrise;
}

time_t getSunset(void) {
	return sunset;
}
